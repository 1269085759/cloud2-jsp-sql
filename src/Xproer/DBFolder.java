...
public class DBFolder {
	...
	static public int Add(FolderInf inf)
	{
		...
		//获取新插入的ID
		String sql = "select top 1 fd_id from cloud2_folders order by fd_id desc";
	...
	{
		String sql = "select sum(cast(f_lenSvr as int)) as lenPosted from (select distinct f_md5,f_lenSvr from cloud2_files where f_pidRoot=? and LEN(f_md5) > 0) a";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try {
